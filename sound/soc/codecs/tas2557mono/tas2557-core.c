...
/* HTC_AUD_START: remove clear firmware due to not reload back */
#if 0
...
/* HTC_AUD_START */
	if (!pFirmware->mpPLLs) {
		dev_err(pTAS2557->dev, "%s: mpPLLs failed to allocate memory.\n", __func__);
		return -ENOMEM;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START */
		if (!pPLL->mpDescription) {
			dev_err(pTAS2557->dev, "%s: mpDescription is out of memory.\n", __func__);
			return -ENOMEM;
		}
/* HTC_AUD_END */
...
/* HTC_AUD_START */
	if (!pFirmware->mpPrograms) {
		dev_err(pTAS2557->dev, "%s: mpPrograms failed to allocate memory.\n", __func__);
		return -ENOMEM;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START */
	if (!pFirmware->mpConfigurations) {
		dev_err(pTAS2557->dev, "%s: mpConfigurations failed to allocate memory.\n", __func__);
		return -ENOMEM;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START */
	if (!pFirmware->mpCalibrations) {
		dev_err(pTAS2557->dev, "%s: mpCalibrations failed to allocate memory.\n", __func__);
		return -ENOMEM;
	}
/* HTC_AUD_END */
	for (nCalibration = 0;
...
/* HTC_AUD_START */
	if (nPosition < 0) {
		dev_err(pTAS2557->dev, "%s: fw_parse_pll_data error %d.\n", __func__, nPosition);
		return nPosition;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START */
	if (nPosition < 0) {
		dev_err(pTAS2557->dev, "%s: fw_parse_program_data error %d.\n", __func__, nPosition);
		return nPosition;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START */
	if (nPosition < 0) {
		dev_err(pTAS2557->dev, "%s: fw_parse_configuration_data error %d.\n", __func__, nPosition);
		return nPosition;
	}
/* HTC_AUD_END */
...
/* HTC_AUD_START */
		if (nPosition < 0) {
			dev_err(pTAS2557->dev, "%s: fw_parse_calibration_data error %d.\n", __func__, nPosition);
			return nPosition;
		}
/* HTC_AUD_END */
	}
