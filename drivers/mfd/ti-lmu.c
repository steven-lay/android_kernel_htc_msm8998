...
static int __init ti_lmu_init(void)
{
	int ret = -ENODEV;

	pr_info("ti_lmu_init\n");
	ret = i2c_add_driver(&ti_lmu_driver);
	if (ret != 0)
		pr_err("Failed to register I2C driver: %d\n", ret);

	return ret;
}
subsys_initcall(ti_lmu_init);

static void __exit ti_lmu_exit(void)
{
	i2c_del_driver(&ti_lmu_driver);
}
module_exit(ti_lmu_exit);
