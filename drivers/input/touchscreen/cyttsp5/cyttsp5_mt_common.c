...
//htc+
		...
		if( tch->abs[CY_TCH_E] == CY_EV_TOUCHDOWN ||
			...
			htc_tamper_fs_print(dev, "Finger #%d(%d) %s => X:%d, Y:%d, Z:%d, N:%d, FR:%d",
				tch->abs[CY_TCH_T],
				tch->hdr[CY_TCH_NUM],
				...
				tch->hdr[CY_TCH_HOPPING_FREQ] //htc: RESERVED FIELD, SAME OFS as NOISE EFFECT
			);
		}
#endif
//htc-
...
static int cyttsp5_mt_resume_attention(struct device *dev)
{
	...
	mutex_lock(&md->mt_lock);
//htc++: report all finger leave when driver resume
	if(md->si) {
