...
struct htc_cyttsp5_mfg_data{
	union {
		u8   u8_mutual_cap_data[4096]; //htc data
		u8   u8_mutual_cap_Tx30_Rx17x2[30][17*2]; //5.5" vendor data
		u8   u8_mutual_cap_Tx28_Rx16x2[28][16*2]; //5.2" vendor data
	};
	...
	int new_line_pos, enum htc_mfg_item item_id)
{
	...
	//special in_idx for self cap Tx line
	else if(in_idx == length) {
		//self cap Tx line
		sum = 0;
		memset(buf, 0, sizeof(htc_string_self_cap));
...
//
// vendor: Tx00, Tx01, TX02, ..., TX16, Rx00, RX01, Rx02, ..., RX29
// htc: Rx00, RX01, Rx02, ..., RX29, Tx00, Tx01, TX02, ..., TX16
//
...
//	pr_info("%s: data_sz %d", __func__, data_sz);
		...
		u8 *out_buf, size_t out_buf_size, int hdr_sz, int new_line_pos, enum htc_mfg_item item_id)
{
	...
	index = 0;
	dad = cyttsp5_get_device_access_data(dev);
	in_data = dad->mfg_data.u8_mutual_cap_data; //htc data pointer
	...
	if( item_id == eHTC_MFG_CM_PANEL &&
		...
		//skip last two bytes - calibration data
...
//
// MUST TAKE CARE THE INPUT DATA POINTER AND DATA SIZE IS DIFFERENT
//
