...
#include <linux/of_gpio.h> //htc: of_get_named_gpio()
...
//htc+
	...
	if (pdata->rst_gpio < 0) {
		pr_debug("%s: no rst_gpio", __func__);
		rc = -EIO;
		goto fail;
	}
	...
	rc = of_property_read_u32(core_node, "cy,support_glove", &value);
	if (rc) {
		pr_info("%s: fail to request glove mode setting\n", __func__);
		pdata->support_glove = 0;
	} else {
		pdata->support_glove = value;
		pr_info("%s: set support glove mode %d\n", __func__, value);
	}
	pdata->TP_mode = 1;
//htc-
