...
/* drivers/soc/qcom/custom_platdev.c
 *
...
static void set_wifi_mac(void)
{
	...
	src = (const char *)(ptr+WLAN_MAC_OFFSET);
	/*Intf0MacAddress*/
...
(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (count > WLAN_MAC_SIZE - p)
		count = WLAN_MAC_SIZE - p;

	ptr = get_wifi_nvs_ram();

	if (ptr != NULL) {
		if (copy_to_user(buf, (void *)
			(ptr+WLAN_MAC_OFFSET+p), count)) {
			ret =  -EFAULT;
			pr_err("wifi_mac_read_proc: FAIL to copy\n");
		} else {
			*ppos += count;
			ret = count;
		}
		...
		ret =  -EFAULT;
		pr_err("wifi_mac_read_proc: ptr is null\n");
	}
...
static int __init wifi_nvs_init(void)
{
	...
	wifi_mac = proc_create_data("calibration", 0444, NULL,
			&wifi_mac_fops, NULL);
	if (wifi_mac == NULL)
		pr_debug("%s: unable to create /proc/calibration entry\n"
		, __func__);
	else
		wifi_mac->size = WLAN_MAC_SIZE;
