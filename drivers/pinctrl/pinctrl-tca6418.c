...
static int htc_IOExpRSTpin_probe(struct platform_device *pdev)
{
	int ret;
	struct pinctrl *rstpin_pinctrl;
	struct pinctrl_state *set_state;

	pr_info("== IOExpander RST pin_probe ==\n");

	// Init rstpin wake_host_pin
	rstpin_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(rstpin_pinctrl)) {
		if (PTR_ERR(rstpin_pinctrl) == -EPROBE_DEFER)
			pr_err("rstpin_pinctrl EPROBE_DEFER !!\n");

		return PTR_ERR(rstpin_pinctrl);
	}

	pr_info("Init IOExpander GPIO pins\n");

	set_state = pinctrl_lookup_state(rstpin_pinctrl, PINCTRL_STATE_DEFAULT);
	if (IS_ERR(set_state)) {
		pr_err("Lookup rstpin pinctrl gpio state error\n");
		return -ENODEV;
	}
	ret = pinctrl_select_state(rstpin_pinctrl, set_state);
	if (ret) {
		pr_err("Set rstpin pinctrl gpio state error :%d\n", ret);
		return -EINVAL;
	}
	return 0;
}

static const struct of_device_id htc_IOExpRSTpin_match_table[] = {
	{ .compatible = "htc,IOExpRST_pin", },
	{},
};

static struct platform_driver htc_IOExpRSTpin_driver = {
	.probe = htc_IOExpRSTpin_probe,
	.driver = {
		.name = "IOExpRST_pin",
		.owner = THIS_MODULE,
		.of_match_table = htc_IOExpRSTpin_match_table,
	},
};
		...
		const struct i2c_device_id *id)
{
	...
	/* register platform driver */
	if (platform_driver_register(&htc_IOExpRSTpin_driver)) {
		dev_err(&client->dev, "Fail to register IOExpRST pin driver\n");
		return -EINVAL;
	}
